...
class GPUCB1_Learner(GP_Learner):
    ...
    def pull_arm(self):
        ...
        if index==-1:
            t = np.sum(self.price_curve.arm_selections) + 1  # Total number of arm selections
            ...
            idx = np.argmax([np.max(sampled_values_dc*ucb_values[i]-sampled_values_cc) for i in range(n_arms)] )
