...
for e in range (n_experiments):
  ...
  env = Environment_Bids(bids, n_arms, prices, p_arms, cost)
  gpts_learner = GPTS_Learner(n_bids, bids, n_arms, cost, prices)
  gpucb_learner = GPUCB1_Learner(n_bids, bids, n_arms, cost, prices)
  ...
  for t in range (T):
    ...
    pulled_bid, cost_click_obs, daily_click_obs, conv_rate_obs, reward  = env.round( pulled_arm)
    gpts_learner.update(pulled_bid, cost_click_obs, daily_click_obs, pulled_arm, conv_rate_obs)
    gpts_collected_reward[t] = reward
    # GP UCB1
    pulled_arm = gpucb_learner.pull_arm()
    pulled_bid, cost_click_obs, daily_click_obs, conv_rate_obs, reward  = env.round(pulled_arm)
    gpucb_learner.update(pulled_bid, cost_click_obs, daily_click_obs, pulled_arm, conv_rate_obs)
...
label  = ['GPTS','GPUCB1']
plot_function(matrix_val, optimal, label, 'Exercise 3 Result')